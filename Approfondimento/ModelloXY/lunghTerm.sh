...
input_file="input1.dat"      		          # Nome del file di input
tempXY=(0.5 1.0)               # Temperature a cui simulo il modello
deltaXY=(1.1 1.7)                 # Rotazione angolare degli spin
...
for ((i=0; i<${#sizeXY[@]}; i++)); do
    ...
    for ((j=0; j<${#tempXY[@]}; j++)); do
        ...
        for ((k=1; k<=4; k++)); do
            ...
            ./modelXY input1.dat analisi/conf/conf_t${tempXY[j]}_size${sizeXY[i]}.out | tee analisi/term/term_t${tempXY[j]}_size${sizeXY[i]}_seed$k.out
