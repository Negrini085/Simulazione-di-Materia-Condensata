...
input_file="input2.dat"      		          # Nome del file di input
tempXY=(1.5 2.0)               # Temperature a cui simulo il modello
deltaXY=(3.2 4.0)                 # Rotazione angolare degli spin
...
for ((i=0; i<${#sizeXY[@]}; i++)); do
    ...
    for ((j=0; j<${#tempXY[@]}; j++)); do
        ...
        for ((k=1; k<=4; k++)); do
            ...
            ./modelXY input2.dat analisi/conf/conf_t${tempXY[j]}_size${sizeXY[i]}.out | tee analisi/term/term_t${tempXY[j]}_size${sizeXY[i]}_seed$k.out
